...
export function updateTotal() {
  ...
  fetchProductsList('computador').then((data) => {
    getSavedCartIDs().forEach((id) => {
      preco.push(data.find((element) => element.id === id).price);
    });
    total.innerText = preco.reduce((acc, cur) => acc + cur, 0);
  });
}
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    saveCartID(id);
    const itens = document.querySelector('.cart__products');
    const parametro = await fetchProduct(id);
    const products = createCartProductElement(parametro);
    itens.appendChild(products);
  });
